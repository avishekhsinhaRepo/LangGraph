...
def divide(op1: int, op2: int) -> float:
    ...
    Args:
        op1 (int): The numerator.
        op2 (int): The denominator.
    Returns:
        float: The result of division.
    Raises:
        ValueError: If op2 is zero.
    """
    if op2 == 0:
...
tools = [add, multiply, divide, wikipedia_search]
...
messages = runnable.invoke(
    {
        "messages": [
            HumanMessage(
                "What is 5 + 3? and the product of 4 and 6? and the division of 20 by 4? Also, who is Sachin Tendulkar?"
            )
        ]
    }
)
